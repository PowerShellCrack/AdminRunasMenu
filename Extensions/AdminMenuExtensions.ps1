...
'
function Toggle-PSConsole {
    if (!($DebugItem.Checked)) {
        $DebugItem.Checked = $true
        $DebugItem.Text = 'Hide Debug Console'
        [System.Windows.Forms.MessageBox]::Show("Use the 'Hide Debug button' in the menu from the taskbar icon to close this debug window.`n`nIf closed with the Exit button, it will close the Admin Run-As Menu as well","Console Warning",
...
function Show-Menu {
    ...
    $App.Show()
    If($OnTop){$App.Activate()| Out-Null}
}

function Hide-Menu {
    Param(
        [Parameter(Mandatory=$false)]
	    [switch]$ShowTip = $false
    )
    $HApp.Hide()
    $OApp.Topmost = $True
    $OApp.Show()
	...
	$App.Hide()
    If($ShowTip){$notifyicon.ShowBalloonTip(5)}
}
...
function Start-ButtonProcess{
    ...
    Begin {
        $originalPath = $Path
        $ExtensionScriptsPath = Join-Path -Path $ExtensionPath -ChildPath "\Tools"
	...
	Process {
		If (($ProcessCall -ne "registry") -and !(Test-Path -LiteralPath $Path -PathType 'Leaf' -ErrorAction 'Stop') ) {
			If ($OutputTab){Write-OutputBox -OutputBoxMessage $errmsg -Type "ERROR: " -Object $OutputTab -LogOutput}
            Return 
		}
		Else {
            Try{
...
                                {$Domain = $RawUserName.Split('\')[0]; $User = $RawUserName.Split('\')[1]} 
            ...
            Catch{
                ...
                If ($OutputTab){Write-OutputBox -OutputBoxMessage "failed to start process. The error message was [$ErrorMessage]" -Type "ERROR: " -Object $OutputTab -LogOutput}
            }
        }
    }
    End {}
...
Function Save-ComputerList {
    ...
    [scriptblock]$AddBtnScript = {
        If ((-NOT [string]::IsNullOrEmpty($inputTextBox.text))) {
            $Exists = Compare-Object -ReferenceObject $inputTextBox.text (Get-Content $SavedPath) -IncludeEqual -passThru | Where-Object { $_.SideIndicator -eq '==' }
            ...
            elseif ($Exists){
                [System.Windows.Forms.MessageBox]::Show('System name already exists.','Already Exists',
                [System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Error)
            }
            else{
